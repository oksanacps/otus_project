...
class TestCreateOwner:
    ...
    @pytest.mark.parametrize("field, value, expected_status", [
        ...
        ("address", "A" * 256, 400),
        ("city", "A" * 81, 400),
        ("telephone", "12345678901", 400),    # Специально, что бы в отчете были падения
    ])
    def test_create_owner_field_length_validation(self, get_request_instance, generate_owner_data, field, value,
                                                  expected_status, cleanup_owner):
        data_new_owner = generate_owner_data
        data_new_owner[field] = value
        response = get_request_instance.post('api/owners', body=json.dumps(data_new_owner))
