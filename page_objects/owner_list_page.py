...
class OwnerListPage(BasePage):
    ...
    def click_owner_name_button(self, owner_name):
        ...
        while attempt < max_attempts:
            try:
                button = self.is_presence((By.XPATH, f'//a[contains(text(), "{owner_name}")]'))
                self.driver.execute_script("arguments[0].scrollIntoView({block: 'center', behavior: 'smooth'});",
                                           button)
                button = self.is_clickable((By.XPATH, f'//a[contains(text(), "{owner_name}")]'))
                button.click()
                return
    ...
    def owner_in_table_is_visible(self, owner_name):
        ...
        while attempt < max_attempts:
            try:
                element = self.is_presence((By.XPATH, f'//a[contains(text(), "{owner_name}")]'))
                self.driver.execute_script("arguments[0].scrollIntoView({block: 'center', behavior: 'smooth'});",
                                           element)
                if element.is_displayed():
                    return True

            except (AssertionError, NoSuchElementException, StaleElementReferenceException):
                self.driver.execute_script("window.scrollBy(0, 300);")
                attempt += 1
                time.sleep(1)
